...
As a native Minnesotan, I'm required to be a Wolves fan, though I fully understand the inevitable heartache and loss that Minnesota sports will bring me. A prominent narrative surrounding the Wolves this year has been the heavy minutes laid on their top 5 and the toll it's been taking on the players. Just looking at minutes played this season, 3 of the top 5 players are from Minnesota.
...
```{r, echo=F, fig.fullwidth=T, fig.width=10, fig.height=6}
...
```{r, echo=F, fig.margin=T, fig.width=6, fig.height=4, fig.cap="The Wolves and Nets are two extremes in distribution of minutes.", warning=FALSE}
p
```
...
p <- d4 %>% 
  filter(rank <= 20) %>% 
  ggplot(aes(x=rank, y=cumulMins, color=Tm.x, alpha=Tm.x,label=label))+
  geom_line(aes(frame=Tm.x),size=0.8)+
  theme_tufte()+
  theme(legend.position="none", text=element_text(family="Roboto Condensed", colour="black", size=20))+
  xlab("Within-Team Rank of Players by Mins")+
  ylab("Cumulative Percentage of Team Mins")+
...
```{r, fig.show="animate", echo=F, fig.width=10, fig.height=6, warning=FALSE, cache=T, error=F, comment=F}
gganimate(p)
```
...
gganimate(p, title_frame = F, filename = "Houston_comparison.gif")
```

```{marginfigure}
![Houston_comparison](Houston_comparison.gif)<br>
```
<br>
<br>
<br>
...
```{marginfigure}
![james+harden](https://media2.giphy.com/media/XPMCrKFznOBQk/giphy-downsized.gif?cid=e1bb72ff5a4e74312e6b63634d3f696b)<br>
```
...
# **Should add a gif plot somewhere in here, maybe changes in a team through the years, something like that?**
